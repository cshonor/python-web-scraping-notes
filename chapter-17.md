...
- **调度**：`cron`、`systemd timer`、Windows 任务计划程序；见 `code/chapter17/run_once.py` 作为最小入口示例。
...
- **`code/chapter17/tor_requests_ip.py`**：`requests` + `socks5h`（依次尝试 **9150 / 9050**）。  
- **`code/chapter17/selenium_tor_chrome.py`**：Chrome `--proxy-server=socks5://...`（需与本机 Tor 端口一致）。
...
`code/chapter17/run_once.py`：供调度器调用的最小「单次任务」壳。
