...
可运行脚本（含**离线双站**演示 + **example.com** 一次线上探测）：`code/chapter04/scraper_model_demo.py`
...
`code/chapter04/crawler_model.py` 演示 **`PageRecord` 数据类 + `fetch`/`parse`/`run` 分层**，可与本课的 `Website`/`Crawler` 模式对照阅读。
