...
可运行脚本（含超时、HTTPS、User-Agent）：`code/chapter06/wikitable_to_csv.py`
...
可运行脚本：`code/chapter06/mysql_pages_demo.py`
...
- **`code/chapter06/save_csv_json.py`**：`DictWriter` + **JSON Lines** 小示例。  
- **`code/chapter06/sqlite_store.py`**：无 MySQL 时的本地 **`sqlite3`** 落库练习。  
- **`code/chapter06/download_urlretrieve.py`**（可选）：演示将远程图片保存到本地（运行生成 `_demo_*.png`，已 `.gitignore`）。
