...
- **Playwright**：微软维护，API 现代、内置浏览器二进制；仓库示例见 `chapter-11/playwright_ajax_demo.py` 与 `rendered_title.py`。
...
- **`chapter-11/selenium_ajax_wait.py`**（需 `pip install selenium`，本机已安装 Chrome）
- **`chapter-11/playwright_ajax_demo.py`**（需 Playwright，作为不依赖 WebDriver 的备选）
...
- `rendered_title.py`：Playwright 打开 `example.com` 的最小示例。  
- `js-debug-notes.md`：XHR 与调试思路。
