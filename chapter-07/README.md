...
示例脚本：`chapter-07/read_text_remote.py`
...
- 另可选用 **`pypdf`**（轻量、易上手），见 `chapter-07/read_pdf.py`（本地文件）。
...
可运行脚本：`chapter-07/csv_stringio_remote.py`
...
可运行脚本：`chapter-07/read_pdf_remote.py`（远程 PDF + `pdfminer.six`）

### 2.3 解析 Word（`.docx`）正文

可运行脚本：`chapter-07/read_docx_zip_bs4.py`
...
- **`chapter-07/read_pdf.py`**：本地 PDF + **`pypdf`**，适合离线实验。
